...
Ns = {'abalonePP': 4177,
      ...
      'crxPP': 651,
      'diabetesPP': 768, }
MIN_INST_SLICES = [.1, .01, .05]
COL_SPLIT_THRESHOLD = [.9, .7, .5, .3, .1]

PYTHON_INT = 'ipython3 --'
EXEC_BIN = 'bin/spstd_model_ha1.py'
OUTPUT_DIR = 'exp/vis-adult/'
VERBOSITY_LEVEL = ' -v 0 '
...
def dump_exp_sh_call(date_string, configs):
    ...
    for i, (dataset, m_p, threshold, prior,
        ...
        # exp_name = os.path.join(str(m_p),
        #                         str(threshold), prior, omega_prior, str(miss_perc), str(seed))
        # CMD += ' --exp-id {}'.format(exp_name)

        # miss_path = os.path.join(DATA_DIR, dataset, 'miss-{}.full.data'.format(miss_perc))
        # miss_path = os.path.join(DATA_DIR, dataset, 'miss',
        #                          str(miss_perc), str(seed), 'miss.full.data')
...
def get_configs():
    config_lists = [DATASETS, MIN_INST_SLICES, COL_SPLIT_THRESHOLD,
                    ...
                    N_ITERS, OMEGA_PRIOR, OMEGA_UNIF_PRIOR, LEAF_OMEGA_UNIF_PRIOR  # , MISS_PERC, SEEDS
                    ]
